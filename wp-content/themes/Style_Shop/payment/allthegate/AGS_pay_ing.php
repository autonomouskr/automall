...
********************************************************************************/
	...
	****************************************************************************/
	...
	/*공통사용*/
	$agspay->SetValue("AgsPayHome",dirname(__FILE__));			//올더게이트 결제설치 디렉토리 (상점에 맞게 수정)
	...
	$agspay->SetValue("ProdNm",iconv("UTF-8","EUC-KR",trim($_POST["ProdNm"])));			//상품명
	...
	/*신용카드사용*/
	$agspay->SetValue("OrdNm",iconv("UTF-8","EUC-KR",trim($_POST["OrdNm"])));			//주문자명
	$agspay->SetValue("OrdPhone",trim($_POST["OrdPhone"]));		//주문자연락처
	$agspay->SetValue("OrdAddr",iconv("UTF-8","EUC-KR",trim($_POST["OrdAddr"])));		//주문자주소 가상계좌추가
	$agspay->SetValue("RcpNm",iconv("UTF-8","EUC-KR",trim($_POST["RcpNm"])));			//수신자명
	$agspay->SetValue("RcpPhone",trim($_POST["RcpPhone"]));		//수신자연락처
	$agspay->SetValue("DlvAddr",iconv("UTF-8","EUC-KR",trim($_POST["DlvAddr"])));		//배송지주소
	...
	/*신용카드(ISP)*/
	$agspay->SetValue("partial_mm",iconv("UTF-8","EUC-KR",trim($_POST["partial_mm"])));		//일반할부기간
	$agspay->SetValue("noIntMonth",iconv("UTF-8","EUC-KR",trim($_POST["noIntMonth"])));		//무이자할부기간
	$agspay->SetValue("KVP_CURRENCY",iconv("UTF-8","EUC-KR",trim($_POST["KVP_CURRENCY"])));	//KVP_통화코드
	$agspay->SetValue("KVP_CARDCODE",iconv("UTF-8","EUC-KR",trim($_POST["KVP_CARDCODE"])));	//KVP_카드사코드
	$agspay->SetValue("KVP_SESSIONKEY",iconv("UTF-8","EUC-KR",$_POST["KVP_SESSIONKEY"]));	//KVP_SESSIONKEY
	$agspay->SetValue("KVP_ENCDATA",iconv("UTF-8","EUC-KR",$_POST["KVP_ENCDATA"]));			//KVP_ENCDATA
	$agspay->SetValue("KVP_CONAME",iconv("UTF-8","EUC-KR",trim($_POST["KVP_CONAME"])));		//KVP_카드명
	$agspay->SetValue("KVP_NOINT",iconv("UTF-8","EUC-KR",trim($_POST["KVP_NOINT"])));		//KVP_무이자=1 일반=0
	$agspay->SetValue("KVP_QUOTA",iconv("UTF-8","EUC-KR",trim($_POST["KVP_QUOTA"])));		//KVP_할부개월
...
<form name="frmAGS_pay_ing" method="post" action="<?php echo $_POST['BBSE_COMMERCE_THEME_WEB_URL']; ?>/proc/order.exec.php">

<!-- 각 결제 공통 사용 변수 -->
<input type="hidden" name="AuthTy" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("AuthTy")); ?>">		<!-- 결제형태 -->
<input type="hidden" name="SubTy" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("SubTy")); ?>">			<!-- 서브결제형태 -->
<input type="hidden" name="rStoreId" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rStoreId")); ?>">	<!-- 상점아이디 -->
<input type="hidden" name="rOrdNo" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rOrdNo")); ?>">		<!-- 주문번호 -->
<input type="hidden" name="rProdNm" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("ProdNm")); ?>">		<!-- 상품명 -->
<input type="hidden" name="rAmt" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rAmt")); ?>">			<!-- 결제금액 -->
<input type="hidden" name="rOrdNm" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("OrdNm")); ?>">		<!-- 주문자명 -->
<input type="hidden" name="rUserEmail" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("UserEmail")); ?>">		<!-- 이메일 -->
<input type="hidden" name="rOrdPhone" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("OrdPhone")); ?>">		<!-- 연락처 -->
...
<input type="hidden" name="rSuccYn" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rSuccYn")); ?>">	<!-- 성공여부 -->
<input type="hidden" name="rResMsg" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rResMsg")); ?>">	<!-- 결과메시지 -->
<input type="hidden" name="rApprTm" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rApprTm")); ?>">	<!-- 결제시간 -->

<!-- 신용카드 결제 사용 변수 -->
<input type="hidden" name="rBusiCd" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rBusiCd")); ?>">		<!-- (신용카드공통)전문코드 -->
<input type="hidden" name="rApprNo" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rApprNo")); ?>">		<!-- (신용카드공통)승인번호 -->
<input type="hidden" name="rCardCd" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rCardCd")); ?>">	<!-- (신용카드공통)카드사코드 -->
<input type="hidden" name="rDealNo" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rDealNo")); ?>">			<!-- (신용카드공통)거래번호 -->

<input type="hidden" name="rCardNm" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rCardNm")); ?>">	<!-- (안심클릭,일반사용)카드사명 -->
<input type="hidden" name="rMembNo" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rMembNo")); ?>">	<!-- (안심클릭,일반사용)가맹점번호 -->
<input type="hidden" name="rAquiCd" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rAquiCd")); ?>">		<!-- (안심클릭,일반사용)매입사코드 -->
<input type="hidden" name="rAquiNm" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rAquiNm")); ?>">	<!-- (안심클릭,일반사용)매입사명 -->

<!-- 계좌이체 결제 사용 변수 -->
<input type="hidden" name="ICHE_OUTBANKNAME" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("ICHE_OUTBANKNAME")); ?>">		<!-- 이체은행명 -->
<input type="hidden" name="ICHE_OUTBANKMASTER" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("ICHE_OUTBANKMASTER")); ?>">	<!-- 이체계좌예금주 -->
<input type="hidden" name="ICHE_AMOUNT" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("ICHE_AMOUNT")); ?>">					<!-- 이체금액 -->

<!-- 핸드폰 결제 사용 변수 -->
<input type="hidden" name="rHP_HANDPHONE" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("HP_HANDPHONE")); ?>">		<!-- 핸드폰번호 -->
<input type="hidden" name="rHP_COMPANY" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("HP_COMPANY")); ?>">			<!-- 통신사명(SKT,KTF,LGT) -->
<input type="hidden" name="rHP_TID" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rHP_TID")); ?>">					<!-- 결제TID -->
<input type="hidden" name="rHP_DATE" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rHP_DATE")); ?>">				<!-- 결제일자 -->

<!-- ARS 결제 사용 변수 -->
<input type="hidden" name="rARS_PHONE" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("ARS_PHONE")); ?>">			<!-- ARS번호 -->

<!-- 가상계좌 결제 사용 변수 -->
<input type="hidden" name="rVirNo" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("rVirNo")); ?>">					<!-- 가상계좌번호 -->
<input type="hidden" name="VIRTUAL_CENTERCD" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("VIRTUAL_CENTERCD")); ?>">	<!--입금가상계좌은행코드(우리은행:20) -->

<!-- 이지스에스크로 결제 사용 변수 -->
<input type="hidden" name="ES_SENDNO" value="<?php echo iconv("EUC-KR","UTF-8",$agspay->GetResult("ES_SENDNO")); ?>">				<!-- 이지스에스크로(전문번호) -->
