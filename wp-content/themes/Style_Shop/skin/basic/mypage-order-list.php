...
/* List Query  */
if($Loginflag=='member'){
	$total = $wpdb->get_var("SELECT COUNT(*) FROM bbse_commerce_order WHERE order_status IN ('PR', 'PE', 'DR', 'DI', 'DE', 'OE', 'EN') AND user_id='".$currUserID."'".$addQuery); //총 목록수
	$total_pages = ceil($total / $per_page); //총 페이지수
	$result = $wpdb->get_results("SELECT * FROM bbse_commerce_order WHERE order_status IN ('PR', 'PE', 'DR', 'DI', 'DE', 'OE' , 'EN') AND user_id='".$currUserID."'".$addQuery.$orderby.$sort." LIMIT ".$start_pos.", ".$per_page);
}
else if($Loginflag=='social' && $currSnsIdx>'0'){
	$total = $wpdb->get_var("SELECT COUNT(*) FROM bbse_commerce_order WHERE order_status IN ('PR', 'PE', 'DR', 'DI', 'DE', 'OE', 'EN') AND user_id='' AND sns_id='".$currUserID."' AND sns_idx='".$currSnsIdx."'".$addQuery); //총 목록수
	$total_pages = ceil($total / $per_page); //총 페이지수
	$result = $wpdb->get_results("SELECT * FROM bbse_commerce_order WHERE order_status IN ('PR', 'PE', 'DR', 'DI', 'DE', 'OE', 'EN') AND user_id='' AND sns_id='".$currUserID."' AND sns_idx='".$currSnsIdx."'".$addQuery.$orderby.$sort." LIMIT ".$start_pos.", ".$per_page);
}
...
<script language="javascript">
	...
	function getDateAdd(f,v,t){ //오늘날짜, 일차(과거:-)
		...
		<h2 class="page_title">주문/배송조회</h2>
		<div class="article">
		...
		<input type="hidden" name="bbseMy" id="bbseMy" value="order-list">
			...
			<dl>
				...
				<dd>
					...
					<div class="bbse_commerce_ui bb_cal_select">
						<label class="blind" for="searchFirstDate">시작 날짜</label><input type="text" class="datepicker" value="<?php echo $V['search_date2'];?>" name="search_date1" id="search_date1" style="width:80px;text-align:center;" readonly/>
						<div class="cal_wrap">
						...
						<button type="button" class="bb_btn shadow" onclick="jQuery('#orderListFrm').submit();"><span class="mid">조회</span></button>
					</div>
				</dd>
			</dl>
		</div>
		...
		<div class="fakeTable marginTop orderListTbl ">
			...
			if($total > 0) {
				foreach($result as $order){
					...
					$detail = $wpdb->get_row("SELECT * FROM bbse_commerce_order_detail WHERE order_no='".$order->order_no."' ORDER BY idx ASC LIMIT 1");
					unset($basicOpt);
					unset($addOpt);
					unset($basicImg);
			...
			<ul>
				<li class="orderdInfoCell">
					...
					<a href="<?php echo esc_url( home_url( '/' ) ); ?>?bbseMy=order-detail&ordno=<?php echo $order->order_no.$addQueryString;?>" class="order_num"><?php echo $order->order_no; ?></a>
					<?php if($order->order_status=="PR" || $order->order_status=="PE" || $order->order_status=="DR") {?>
					<button type="button" class="bb_btn shadow orderCancel_open" data-ordercancel="<?php echo $order->order_no; ?>|||<?php echo $detail->goods_name; ?><?php if($orderCnt>1){echo " 외 ".number_format($orderCnt-1)."건";}?>|||<?php echo $order->order_status; ?>"><span class="sml">주문취소</span></button>
					<?php }?>
				</li>
				<li class="goodsInfoCell">
					<div class="goodsBaseInfo">
						<a href="<?php echo esc_url( home_url( '/' ) ); ?>?bbseMy=order-detail&ordno=<?php echo $order->order_no.$addQueryString;?>"><img src="<?php echo $basicImg['0'];?>" alt="<?php echo $detail->goods_name; ?>"></a>
						<a href="<?php echo esc_url( home_url( '/' ) ); ?>?bbseMy=order-detail&ordno=<?php echo $order->order_no.$addQueryString;?>" class="subj">
				...
				?>
					...
					<button type="button" class="bb_btn cus_fill order_confirm" data-orderend="<?php echo $order->order_no; ?>|||<?php echo $detail->goods_name; ?><?php if($orderCnt>1){echo " 외 ".number_format($orderCnt-1)."건";}?>|||<?php echo $order->order_status; ?>"><span class="sml">구매확정</span></button>
					<button type="button" class="bb_btn shadow openLayer" data-ordertracking="<?php echo $order->order_no; ?>" data-name="tracking" data-tracking="<?php echo $order->order_no;?>^<?php echo $detail->goods_name; ?><?php if($orderCnt>1){echo " 외 ".number_format($orderCnt-1)."건";}?>^<?php echo $order->delivery_company;?>^<?php echo $order->delivery_no;?>^<?php echo $order->delivery_url;?>"><span class="sml">배송조회</span></button>
<!-- 					<button type="button" class="bb_btn shadow recall_open" data-orderrefund="<?php echo $order->order_no; ?>|||<?php echo $detail->goods_name; ?><?php if($orderCnt>1){echo " 외 ".number_format($orderCnt-1)."건";}?>"><span class="sml">반품신청</span></button> -->
					<?php }else if($order->order_status=="OE") {?>
					<button type="button" class="bb_btn shadow openLayer" data-ordertracking="<?php echo $order->order_no; ?>" data-name="tracking" data-tracking="<?php echo $order->order_no;?>^<?php echo $detail->goods_name; ?><?php if($orderCnt>1){echo " 외 ".number_format($orderCnt-1)."건";}?>^<?php echo $order->delivery_company;?>^<?php echo $order->delivery_no;?>^<?php echo $order->delivery_url;?>"><span class="sml">배송조회</span></button>
					<?php }?>
				</li>
				<li class="orderStatusInfoCell">
					<span class="orderStatus <?php echo $statusClass?>" style="text-align:center"><?php if($order->pay_status == 'PW'){echo "정산대기";}else if($order->pay_status == 'EN'){ echo "정산완료";}?></span>
				</li>
			</ul>
			<?php
				}
	  ...
	  <form>
		  ...
		  <input type="hidden" name="page_kind" id="page_kind" value="list">
     ...
     <div id="refundBlock" class="article recall_box">
		...
		<form name="orderRefundFrm" id="orderRefundFrm" method="post">
          ...
          <tr>
            ...
            <td id="refund_order_no"></td>
          </tr>
          <tr>
            <th scope="row">상품명</th>
            <td id="refund_goods_name"></td>
          </tr>
          </tr>
          <tr>
	  ...
	  <div id="cancelBlock" class="article orderCancel_box">
	    ...
	    <form name="orderCancelFrm" id="orderCancelFrm" method="post">
          ...
          <tr>
            <th scope="row">주문번호</th>
            <td id="cancel_order_no"></td>
          </tr>
          <tr>
            <th scope="row">상품명</th>
            <td id="cancel_goods_name"></td>
          </tr>
          <tr id="refund_bank_info_view">
            <th scope="row">환불계좌 <em class="c_point">*</em></th>
            <td>
				<p class="my_att_desc"><em>*</em> 결제방법에 관계없이 PG사 취소 승인기간이 지났을 경우 필요한 내용입니다.</p>
				<table class="refund_input">
