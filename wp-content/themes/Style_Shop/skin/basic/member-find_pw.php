...
<div class="article">
	<div class="bd_box">
		<div class="idpwFindBox">
			<p>회원가입 할 때 등록한 이메일 주소로 임시비밀번호가 발송됩니다.</p>
			<div class="inputContainer">
				<label for="user_id">아이디</label><input type="text" name="user_id" id="user_id" onkeydown="if(jQuery(this).val() != ''){jQuery('#error_box').empty().hide();}"  /><br>
			...
			<p class="btm_area">
				...
				<button type="button" class="bb_btn cus_fill shadow" onclick="pass_search('<?php echo $action_url?>/proc/pass.search.php');"><span class="mid">비밀번호찾기</span></button>
			</p>
		</div><!--//회원로그인 -->
	</div>
</div>
