...
$payHowTitle="간편결제 (<img src='".BBSE_COMMERCE_PLUGIN_WEB_URL."/images/ezpay_kakaopay.png' class='ezpay_icon' align='absmiddle' alt='KakaoPay 결제' />)";
...
<?php 
/* * ************************
 * 1. KakaoPay 라이브러리 인클루드 *
 * ************************ */
require(BBSE_COMMERCE_THEME_ABS_PATH."/payment/KakaoPay/lib/lgcns_CNSpay.php");
...
$serveraddr = ($_SERVER['SERVER_ADDR'])?$_SERVER['SERVER_ADDR']:$_SERVER['LOCAL_ADDR'];
$ediDate = $ediDate;
...
?>
		...
		<!--jQuery 사용안함 : kakaopayDlp.pure.min.js, JQuery 사용 : kakaopayDlp.min.js-->
		<script type="text/javascript" src="<?php echo ($CNSPAY_WEB_SERVER_URL) ?>/js/dlp/client/kakaopayDlp.pure.min.js" charset="utf-8"></script>
		...
		<script type="text/javascript">
			...
			*/
			function cnspay() {
				...
				if(document.payForm.resultCode.value == '00') {
					...
					kakaopayDlp.setTxnId(document.payForm.txnId.value);
						...
						kakaopayDlp.addRequestParams({ MOBILE_NUM : orderHP});
			...
			function pay() {
				var data = jQuery("#payForm").serialize();
				jQuery.ajax({
					type: 'post', 
					async: false, 
					url: '<?php echo BBSE_COMMERCE_THEME_WEB_URL?>/payment/KakaoPay/KakaoPay_getTxnId.php', 
					...
					success: function(data){
						...
						if(result['0'] == "success"){
							var url = result['1'];
							var tid = result['2'];
							var win = window.open(url,'','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,left=100,top=100');
        					//win.document.write('<iframe width=100%, height=550 src="'+url+'" frameborder=0 allowfullscreen></iframe>');

							//document.payForm.action = "<?php echo BBSE_COMMERCE_THEME_WEB_URL.'/payment/KakaoPay/KakaoPay_pay_ing.php';?>";

							//cnspay();							
						}
						else{
							alert("txnId 값 수신에 실패하였습니다.   ");
						}
					}
				});	
			}
