<?php
    ...
    if(empty($enc_data)) $enc_data = $_GET['EncodeData'];
	...
	if($conninfo) $sqlStr="auth_ci='".$conninfo."'";
	else $sqlStr="auth_di='".$dupinfo."'";

	$mCnt = $wpdb->get_var("SELECT COUNT(*) FROM bbse_commerce_membership WHERE ".$sqlStr);
...
<head>
	...
	jQuery(document).ready(function() {
		jQuery(opener.document).find("#auth_result").val(jQuery("#auth_result").val());
		jQuery(opener.document).find("#join_frm").submit();
		self.close();
	});
	</script>
</head>
<body>
<form name="checkplusFrm" id="checkplusFrm">
<input type="hidden" name="auth_result" id="auth_result" value="<?php echo base64_encode(serialize($authResult)); ?>">
</form>
</body>
</html>