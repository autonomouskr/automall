...
$recommend = $wpdb->get_var("select display_goods from bbse_commerce_display where display_type='today'");
...
$sort_page = (!$V['sort_page'])?"recommend":$V['sort_page'];
...
/* List Query  */
if($recommend) {
    //$total = $wpdb->get_var("SELECT COUNT(*) FROM bbse_commerce_goods WHERE idx IN(".implode(",",$goods_type_list).") AND (goods_display='display' OR goods_display='soldout')".$addQuery); //총 목록수
    $total = $wpdb->get_var("SELECT COUNT(*) FROM bbse_commerce_goods WHERE idx IN(".implode(",",$goods_type_list).") AND (goods_display='display' OR goods_display='soldout')"); //총 목록수
	$total_pages = ceil($total / $per_page); //총 페이지수
	//$result = $wpdb->get_results("SELECT * FROM bbse_commerce_goods WHERE idx IN(".implode(",",$goods_type_list).") AND (goods_display='display' OR goods_display='soldout')".$addQuery.$orderby.$sort." LIMIT ".$start_pos.", ".$per_page);
	$result = $wpdb->get_results("SELECT * FROM bbse_commerce_goods WHERE idx IN(".implode(",",$goods_type_list).") AND (goods_display='display' OR goods_display='soldout')".$orderby.$sort." LIMIT ".$start_pos.", ".$per_page);
}else $total = 0;
...
?>
		<!-- 상품 목록 -->
		<br /><h2 class="page_title">오늘만특가</h2>
		<div class="article">
...
<!--					<li<?php if($sort_page=="recommend"){?> class="active col4"<?php }else{?> class="col4"<?php }?>><button type="button" onclick="location.href='<?php echo $curURL?>&sort_page=recommend';"><span>오늘만특가</span></button></li>
