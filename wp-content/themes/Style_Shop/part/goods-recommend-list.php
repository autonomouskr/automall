...
if($recommend) {
	...
	//$query = ("SELECT * FROM bbse_commerce_goods WHERE idx IN(".implode(",",$goodsList).") AND  (goods_display='display' OR goods_display='soldout') ".$addQuery.$orderby.$sort." LIMIT ".$start_pos.", ".$per_page);
...
?>
		...
		<br /><h2 class="page_title"><?php echo get_option($theme_shortname."_goodsplace_title_1");//추천상품?></h2>
		<!-- 상품 목록 -->
		<div class="article">
...
<!-- 				<ul class="sorting_tabs"> -->
				<!--N: 활성화는 .active -->
					<!-- <li<?php if($sort_page=="recommend"){?> class="active col4"<?php }else{?> class="col4"<?php }?>><button type="button" onclick="location.href='<?php echo $curURL?>&sort_page=recommend';"><span>추천순</span></button></li>
					...
					if($total > 0) {
						foreach($result as $goods){
						    ...
						    if($role != 'administrator'){
						        ...
						        for($k=0;$k<sizeof($memPrice[goods_consumer_price]); $k++){
