...
<body>
		<?php
			...
			$myInfo = $bbseMember = $wpdb->get_row("
				SELECT 	A.order_name AS name,
						A.order_hp AS hp,
						A.order_phone AS phone,
						A.order_email AS email
				FROM 	bbse_commerce_order AS A
				WHERE 	A.order_no = '".$order_id."'
			");
			$total_array = array();
			$goods_c = '';
			$g_idx = '';
			...
			if(count($total_array) > 0){
				...
				if($where != ''){
					$result2 = $wpdb->get_results("
						SELECT c_name
						FROM bbse_commerce_category
						WHERE (".$where.") AND depth_2 > 0 AND depth_3 = '0'");
		...
		?>
			...
			<script>
			   ...
			   $('.print_c').click(function(){
				...
				function save_ajax(){
					$.ajax({
						url:'<?php echo BBSE_THEME_WEB_URL; ?>/estimate_proc2.php',
						type: 'post',
