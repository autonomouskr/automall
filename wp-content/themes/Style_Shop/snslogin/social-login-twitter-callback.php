...
if (!empty($_GET['oauth_verifier']) && !empty($_SESSION['oauth_token']) && !empty($_SESSION['oauth_token_secret'])) {
    ...
    $twitteroauth = new TwitterOAuth($twt_Config['consumerKey'], $twt_Config['consumerSecret'], $_SESSION['oauth_token'], $_SESSION['oauth_token_secret']);
    $access_token = $twitteroauth->getAccessToken($_GET['oauth_verifier']); // Access token 요청
    $_SESSION['access_token'] = $access_token; // Access token을 세션에 저장
    $userInfo = $twitteroauth->get('account/verify_credentials'); // 사용자 정보 얻기
		...
		<div style="width:95%;margin:0 auto;">
			<div class="socialLogin_msg">
				...
				<div class="socialMsg_content">
					...
					<?php }else{?>
						현재 로그인 하신 <span class="socialLogin_ok">[ 트위터 ]</span>의 이메일 주소와 일치하는 회원정보가 존재합니다.<br />
			...
			<form method="post" name="login_frm" id="login_frm">
				...
				<?php
				/*
				echo "[Google Callback Result]<br /><br /><pre>";
				print_r($userInfo);
				echo "</pre>";
				*/
				?>
