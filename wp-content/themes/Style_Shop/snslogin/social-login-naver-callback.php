...
$bbseSiteType=$_SESSION['bbse_view_site_type'];
$pointColor=(get_option($theme_shortname."_".$bbseSiteType."color_global_background"))?get_option($theme_shortname."_".$bbseSiteType."color_global_background"):$bbseThemmeColorSet['black_gold']['global_background'];
...
$request->get_accesstoken($_REQUEST['code'],$_REQUEST['state']); // Access token 받기
$userInfo=$request->get_user_info(); // Access token을 이용한 사용자 정보 받아오기
//print_r($userInfo);

if($userInfo['sns_id']){ // DB 및 SESSION 저장
	$_SESSION['snsLogin']="Y";
...
<head>
	...
	<style>
		...
		.page_cont .page_title {width:100%;padding: 15px 0 8px 0;height:35px;text-align:center;color:#ffffff;background-color:<?php echo $pointColor?>;font-size: 19px;}
		.socialLogin_msg {width:100%;background-color:#EEEEEE;line-height:20px;margin-bottom:40px;border-radius: 2em 0 2em 0;}
		.socialLogin_msg .socialMsg_title {border:2px solid <?php echo $pointColor?>;color:<?php echo $pointColor?>;margin:0 auto;margin-top:10px;margin-bottom:20px;font-size:15px;text-align:center;width:150px;height:25px;font-weight:700;padding-top:6px;}
		.socialLogin_msg .socialMsg_content {width:90%;margin:0 auto;padding-bottom:20px;font-size:12px;}
		...
		.bd_box .bb_section, .bd_box .single.bb_section {margin: 0px auto 0;width:320px;}
		...
		.socialLoginNext .article .bd_box {font-size:12px;padding:10px;text-align:left;line-height:25px;font-size:13px;}

		.integrationLogin {margin-top:30px;display:none;}
		.integrationLogin .article .bd_box {padding:0 0 20px 0;}
		.bd_box .bb_section .open_alert {margin:15px 0 0 0;}
		.bd_box .bb_section .bb_login_wrap .login_left label {width:70px;}
	</style>
	<script>
		...
		<div style="width:95%;margin:0 auto;">
			<div class="socialLogin_msg">
				...
				<div class="socialMsg_content">
					...
					<?php }else{?>
						현재 로그인 하신 <span class="socialLogin_ok">[ 네이버 ]</span>의 이메일 주소와 일치하는 회원정보가 존재합니다.<br />
			...
			<form method="post" name="login_frm" id="login_frm">
				...
				<?php
				/*
				echo "[Naver Callback Result]<br /><br /><pre>";
				print_r($userInfo);
				echo "</pre>";
				*/
				/*
				[userID] => skqkfk3@naver.com
				[name] => 서재윤
				[email] => skqkfk3@naver.com
				[gender] => M
				*/
...
elseif($rtnFlag=='codeError'){ // 에러
	echo "<script>alert('네이버 아이디로 로그인에 실패하였습니다.');self.close();</script>";
}
?>
	</div>
</body>
