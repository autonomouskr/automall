...
<input type="hidden" name="old_image" id="old_image" value="<?php echo $brdData->image_file?>" />
...
<div id="bbse_board" style="width:<?php echo $table_width?>;<?php echo $table_align?>">
	...
	<fieldset>
		...
		<div class="form_table">
			...
			<?php if(!empty($boardInfo->formtype) && ($boardInfo->formtype == 2 || $boardInfo->formtype == 3)){?>
			<tr>
				<th scope="row">대표이미지</th>
				<td>
					<div class="item">
						<input name="image_file" id="image_file" title="대표이미지" type="file" /><br />
						<input type="checkbox" name="use_content_image" id="use_content_image" value="1"<?php if(!empty($brdData->use_content_image) && $brdData->use_content_image == 1) echo " checked";?> /> <label for="use_content_image">대표이미지 본문노출</label>
						<?php if(!empty($view_image)) echo $view_image?>
					</div>
				</td>
			</tr>
			<tr>
				...
				<td>
					<div class="item">
						<input name="image_file_alt" id="image_file_alt" value="<?php if(!empty($brdData->image_file_alt)) echo $brdData->image_file_alt?>" title="대체텍스트" class="i_text" type="text" style="width:80%;" onkeyup="tag_check(this);" /><br />
