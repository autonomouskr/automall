...
elseif($V['mType']=='step02'){
	...
	$inResult=bbse_commerce_storage_array_in_db($dbData);
	@unlink($r_attach_new);

	$failCount=$inResult['errCnt_null_id']+$inResult['errCnt_duplicate_id']+$inResult['errCnt_null_managerId']+$inResult['errCnt_null_storageName']+$inResult['errCnt_null_businessNumber']+$inResult['errCnt_null_zipcode']+$inResult['errCnt_null_addr1']+$inResult['errCnt_null_addr2']; // 실패 개수
...
elseif($V['mType']=='failCsvMake'){ 
	...
	$header=Array("창고명","우편번호","주소1","주소2","담당자ID");
