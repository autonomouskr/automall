...
if($V['mType']=='step01'){
	...
	$r_file_new="bbse_locaiton_data_".date("YmdHis",$now_time).".".$file_type;
	...
	$str_result = wp_remote_get(BBSE_COMMERCE_UPLOAD_BASE_URL."/bbse-commerce/"."bbse_locaiton_data_".date("YmdHis",$now_time).".".$file_type);
...
elseif($V['mType']=='step02'){
	...
	$inResult=bbse_commerce_location_array_in_db($dbData);
	@unlink($r_attach_new);

	$failCount=$inResult['errCnt_null_id']+$inResult['errCnt_duplicate_id']+$inResult['$errCnt_null_rack_code']+$inResult['$errCnt_null_location_y']+$inResult['$errCnt_null_location_x']+$inResult['$errCnt_null_storageCode']; // 실패 개수
