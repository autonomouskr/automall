	...
	<script language="javascript">
		...
		function select_storage(){
			...
			jQuery("input[name=check\\[\\]]:checked").each(function(){
                storage_name = jQuery(this).siblings('#storage_name').val();
                storage_code = jQuery(this).siblings('#storage_code').val();
                addr1 = jQuery(this).siblings('#storage_addr1').val();
                addr2 = jQuery(this).siblings('#storage_addr2').val();
            }); 
            
            parent.document.getElementById("storage_name").value = storage_name
            parent.document.getElementById("storage_code").value = storage_code
            parent.document.getElementById("storageIdx").value = idx
            parent.document.getElementById("addr1").value = addr1
            parent.document.getElementById("addr2").value = addr2
		...
		function go_search(sType){
			...
			window.location.href ="<?php echo BBSE_COMMERCE_PLUGIN_WEB_URL?>/admin/bbse-commerce-storage-popup.php.?"+strPara;
			//window.location.href ="http://localhost/wp-content/plugins/BBSe_Commerce/admin/bbse-commerce-storage-popup.php.?"+strPara;
		}
...
<body>
	<div class="wrap">
		<div id="popup-goods-list">
			...
			if($searhCon == 'storage_name'){
			    $sOption .=" AND storage_name LIKE %s";
			    $prepareParm[]="%".like_escape($search)."%";
			}
			
			if($searhCon == 'storage_code'){
			    $sOption .=" AND storage_code LIKE %s";
			    $prepareParm[]="%".like_escape($search)."%";
			}
			...
			$sql  = $wpdb->prepare("SELECT * FROM autopole3144.tbl_storage WHERE delete_yn != 'Y' ".$addQuery.$sOption." ORDER BY idx DESC LIMIT %d, %d", $prepareParm);
			$result = $wpdb->get_results($sql);
			
			$s_total_sql  = $wpdb->prepare("SELECT count(*) FROM autopole3144.tbl_storage WHERE delete_yn = 'N' ".$addQuery.$sOption, $prepareParm);
			...
			?>
				...
				<option value="bzNm">창고명</option>
				<option value="bzId">창고코드</option>
			</select>
			...
			?>
				<tr>
					<td style="text-align:center;">
						<input type="hidden" name="pop_storage_name_<?php echo $data->idx;?>" id="pop_storage_name_<?php echo $data->idx;?>" value="<?php echo $data->idx;?>" />
						...
						<input type="hidden" name="storage_name" id="storage_name" value="<?php echo $data->storage_name?>" />
						<input type="hidden" name="storage_code" id="storage_code" value="<?php echo $data->storage_code?>" />
						<input type="hidden" name="storage_addr1" id="storage_addr1" value="<?php echo $data->addr1?>" />
						<input type="hidden" name="storage_addr2" id="storage_addr2" value="<?php echo $data->addr2?>" />
					</td>
					<td style="text-align:center;"><?php echo $data->storage_name?></td>
			...
			?>
				<tr>
					<td colspan="4" style="text-align:center;height:72px">조회된 창고가 없습니다.</td>
				</tr>
			<?php
			}
			?>
			...
			<div style="float:right;margin-right:5px;">
				<button type="button" name="productSelect" id="productSelect" class="button-bbse blue" onClick="select_storage();" style="width:100px;"> 선택 </button>
			</div>
			<div class="clearfix"></div>
