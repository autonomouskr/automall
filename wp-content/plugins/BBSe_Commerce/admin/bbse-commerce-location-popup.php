...
$TB_iframe=true;
$storageCode = $_REQUEST['p_storageCode'];
...
<head>
	...
	<script language="javascript">
		...
		function choose_location(){
			...
			if(chked < 1) {
				alert("물류창고 상세위치를 선택해주세요.");
				return;
			}
			
			if(chked > 1) {
				alert("물류창고 상세위치는 1건만 선택 가능합니다.");
				return;
			}
			...
			jQuery("input[name=check\\[\\]]:checked").each(function(){
                ...
                rackCode = jQuery(this).siblings('#rack_code').val();
                locationX = jQuery(this).siblings('#location_x').val();
                locationY = jQuery(this).siblings('#location_y').val();
            }); 
            
            parent.document.getElementById("rack_code").value = rackCode +"" + locationX + "" + locationY;
...
<body>
	<div class="wrap">
		<div id="popup-goods-list">
			...
			$sql  = $wpdb->prepare("SELECT * FROM autopole3144.tbl_locations WHERE delete_yn != 'Y' and storage_code = '".$storageCode."' ".$addQuery.$sOption." ORDER BY idx DESC LIMIT %d, %d", $prepareParm);
			$result = $wpdb->get_results($sql);
			
			$s_total_sql  = $wpdb->prepare("SELECT count(*) FROM autopole3144.tbl_locations WHERE delete_yn = 'N' and storage_code = '".$storageCode."' ".$addQuery.$sOption, $prepareParm);
			...
			?>
				<tr>
					<td style="text-align:center;">
						...
						<input type="hidden" name="rack_code" id="rack_code" value="<?php echo $data->rack_code;?>" />
						<input type="hidden" name="location_x" id="location_x" value="<?php echo $data->location_x;?>" />
						<input type="hidden" name="location_y" id="location_y" value="<?php echo $data->location_y;?>" />
					</td>
			...
			<div style="float:right;margin-right:5px;">
				<button type="button" name="productSelect" id="productSelect" class="button-bbse blue" onClick="choose_location();" style="width:100px;"> 선택 </button>
			</div>
			<div class="clearfix"></div>
