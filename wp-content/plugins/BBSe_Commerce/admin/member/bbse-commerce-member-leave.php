...
function delete_list(tNo){
	...
	if(tNo > 0){
		if(confirm('해당 회원을 영구적으로 삭제하시겠습니까?   ')){
			frm.delNo.value = tNo;
			frm.mvrun.value = "leave_proc";
			frm.action = 'admin.php?page=bbse_commerce_member&cType=leave';
			frm.submit();
		}
	}
}
function restore_list(tNo){
	var frm = document.join_list_frm;
	if(tNo > 0){
		if(confirm('해당 회원을 복구하시겠습니까?   ')){
			frm.delNo.value = tNo;
			frm.mvrun.value = "restore_proc";
			frm.action = 'admin.php?page=bbse_commerce_member&cType=leave';
			frm.submit();
		}
	}
...
function delete_check_list(sNo){
	...
	if(eval("frm.tBatch" + sNo).value=="leave") {
		msg = "영구적으로 삭제";
		proc = "leave_proc";
	}else if(eval("frm.tBatch" + sNo).value=="restore") {
		msg = "복구";
		proc = "restore_proc";
	}
	...
	jQuery("#join_list_frm").submit();
				...
				<tr>
					...
					<td scope="col" class="manage-column">
						<select id='search2' name='search2' style="width:100%;">
							<option value="leave_date" <?=($search2=="leave_date")?"selected":""?>>탈퇴일</option>
				...
				<div style="margin-bottom:30px;">
					<ul class='title-sub-desc'>
						<li class="current"><a title='전체 목록 보기' href="admin.php?page=bbse_commerce_member&cType=leave">전체(<?php echo number_format($total)?>)</a></li>
					</ul>
				</div>
					...
					<ul class='title-sub-desc none-content' style="float:left;">
						<li>
							<select name="tBatch1">
								...
								<option value="leave">선택 영구삭제</option>
								<option value="restore">선택 회원 복구</option>
							</select>
				...
				<colgroup>
					<col width="35px"/><col width="70px" /><col width="170px" /><col width="140px" /><col width="120px" /><col width="150px" /><col width="100px" /><col width="105px" /><col width="" />
				</colgroup>
				<thead>
				<tr>
					<th scope="col" class="manage-column column-cb check-column" align="center">&nbsp;&nbsp;<input type="checkbox"></th>
					<th scope="col" class="manage-column <?php echo $sorted1?> <?php echo $order?>"><a href="<?php echo $_SERVER['PHP_SELF']?>?page=bbse_commerce_member&#038;cType=leave&#038;orderby=user_no&#038;order=<?php echo $order?>"><span>번호</span><span class="sorting-indicator"></span></a></th>
					<th scope="col" class="manage-column <?php echo $sorted2?> <?php echo $order?>"><a href="<?php echo $_SERVER['PHP_SELF']?>?page=bbse_commerce_member&#038;cType=leave&#038;orderby=user_id&#038;order=<?php echo $order?>"><span>아이디</span><span class="sorting-indicator"></span></a></th>
					<th scope="col" class="manage-column <?php echo $sorted3?> <?php echo $order?>"><a href="<?php echo $_SERVER['PHP_SELF']?>?page=bbse_commerce_member&#038;cType=leave&#038;orderby=name&#038;order=<?php echo $order?>"><span>이름</span><span class="sorting-indicator"></span></a></th>
					<th scope="col" class="manage-column <?php echo $sorted4?> <?php echo $order?>"><a href="<?php echo $_SERVER['PHP_SELF']?>?page=bbse_commerce_member&#038;cType=leave&#038;orderby=user_class&#038;order=<?php echo $order?>"><span>등급</span><span class="sorting-indicator"></span></a></th>
					<th scope="col" class="manage-column">이메일</th>
					<th scope="col" class="manage-column">휴대폰</th>
					<th scope="col" class="manage-column <?php echo $sorted5?> <?php echo $order?>"><a href="<?php echo $_SERVER['PHP_SELF']?>?page=bbse_commerce_member&#038;cType=leave&#038;orderby=leave_date&#038;order=<?php echo $order?>"><span>탈퇴일</span><span class="sorting-indicator"></span></a></th>
					<th scope="col" class="manage-column">탈퇴사유</th>
				</tr>
				</thead>
				<tfoot>
				<tr>
				...
				<tr <?php echo $alternate_class?> valign="top">
					<td scope="row" class="check-column">&nbsp;&nbsp;<input type="checkbox" name="check[]" value="<?=$data->user_no?>"></td>
					<td style="padding-left:10px;"><?php echo number_format($num)?></td>
					<td style="padding-left:10px;">
						<strong><a href="admin.php?page=bbse_commerce_member&cType=leave<?php echo "&user_no=".$data->user_no?>&mode=edit&paged=<?php echo $paged?>&<?php echo $queryString?>" title="회원정보 편집하기"><?php echo stripslashes($data->user_id)?></a></strong>
						<div class="row-actions">
							...
							<span class='trash'><a class='' title='이 회원을 복구' href="javascript:restore_list('<?=$data->user_no?>');">복구</a></span>
						</div>	
					</td>
					...
					<td style="padding-left:10px;"><?php echo ($data->leave_date)?date("Y-m-d H:i:s", $data->leave_date):"-"?></td>
					<td><?php echo ($data->admin_del=='1')?"<span style='color:red;'>관리자 삭제<span>":(($data->leave_reason)?$data->leave_reason:"-")?></td>
				</tr>
				<?php 
					}
				}
