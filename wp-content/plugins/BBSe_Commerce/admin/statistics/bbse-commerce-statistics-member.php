	...
	<div style="margin-top:20px;">
		...
		<div style="width:100%;">
			<table class="dataTbls normal-line-height collapse">
				<colgroup><col width="7%"><col width="20%"><col width="19%"><col width="18%"><col width="18%"><col width="18%"></colgroup>
				<tr>
					<th>번호</th>
					<th>회원명</th>
					<th>주문건수</th>
		...
		if($s_total>'0'){
			...
			$result = $wpdb->get_results($wpdb->prepare("SELECT user_id,count(*) AS cnt,sum(goods_total) AS goods_total,sum(use_earn) AS use_earn,sum(cost_total) AS cost_total FROM bbse_commerce_order WHERE idx<>''".$sOption." GROUP BY user_id ORDER BY cnt DESC LIMIT %d",Array('100')));
		...
		?>
				<tr>
					...
					<td style="text-align:center;"><span class="emBlue">(+) <?php echo number_format($data->goods_total);?>원</span></td>
					<td style="text-align:center;"><span class="emRed">(-) <?php echo number_format($data->use_earn);?>원</span></td>
					<td style="text-align:center;"><span class="emBlue">(+) <?php echo number_format($data->cost_total);?>원</span></td>
				</tr>
	<?php
			}
		}
