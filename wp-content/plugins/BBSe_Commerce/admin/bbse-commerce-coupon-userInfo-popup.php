...
<head>
	...
	<script language="javascript">
		...
		function select_goods(){ 
			let bzName = "<?php echo $_REQUEST['bzName'];?>";
			var chked=jQuery("input[name=check\\[\\]]:checked").not(':disabled').size();
			...
			let str = "";
			//if(bzName != null && bzName != "" && bzName != "undefined" ){
    			for(i=0;i<chked;i++){
    				tUserId=jQuery("input[name=check\\[\\]]:checked").not(':disabled').eq(i).val();
    				
    				userInfoArr = tUserId.split(',');
    				
    				str +="<li id=\"user_info_list_"+userInfoArr[1]+"\" name=\"user_info_list[]\">"
    					+"	            <input style=\"float:left\"; type=\"text\" name=\"coupon_user_list[]\" value=\""+userInfoArr[0]+"\" disabled />"
    					+"              <img style=\"float:left\"; src=\"<?php echo BBSE_COMMERCE_PLUGIN_WEB_URL?>images/btn_delete.png\" onClick=\"goods_user_remove('"+userInfoArr[1]+"')\" class=\"deleteBtn\" alt=\""+userInfoArr[1]+" 회원 삭제\" title=\""+userInfoArr[1]+" 회원 삭제\" /></span>"
    					+"				<div class=\"goodsname\"><input type=\"hidden\" name=\"coupon_user_list[]\" value=\""+userInfoArr[1]+"\" /></div>"
    					+"</li>";
    			}
    			jQuery(parent.document).contents().find("#goods-user-list").append(str);
			//}else{
				
				//jQuery(parent.document).contents().find("#bzName").val(checkValue);
			//}
			parent.remove_popup();
		}
		...
		function go_search(sType){
			...
			window.location.href ="<?php echo BBSE_COMMERCE_THEME_WEB_URL?>/admin/bbse-commerce-coupon-userInfo-popup.php.?"+strPara;
			//window.location.href ="http://localhost/wp-content/plugins/BBSe_Commerce/admin/bbse-commerce-coupon-userInfo-popup.php.?"+strPara;
		}
...
<body>
	<div class="wrap">
		<div id="popup-goods-list">
			...
			$sql  = $wpdb->prepare("SELECT * FROM bbse_commerce_membership WHERE leave_yn <>'1' ".$addQuery.$sOption." ORDER BY user_no DESC LIMIT %d, %d", $prepareParm);
			$result = $wpdb->get_results($sql);
			
			$s_total_sql  = $wpdb->prepare("SELECT count(*) FROM bbse_commerce_membership WHERE leave_yn<>'1' ".$addQuery.$sOption, $prepareParm);
