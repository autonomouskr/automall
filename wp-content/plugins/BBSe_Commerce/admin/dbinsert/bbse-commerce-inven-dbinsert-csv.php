...
?>
	<div class="borderBox">
		<a href="https://autonomouskr.shop/wp-content/uploads/bbse-commerce/bbse_inven_csv_sample.zip"><button type="button" class="button-small red" style="height:25px;margin-left:50px;float:right;">제품 일괄 등록 CSV 샘플 다운로드</button></a>
		 * 제품 일괄 등록은 BBS e-Commerce의 DB 환경에 맞도록 CSV 파일을 작성하신 후 업로드를 통해 제품 관리 정보를  일괄적으로 등록하는 작업을 진행합니다.<br/>
	...
	<div class="memberStep01">
		<form name="dbUploadFrm" id="dbUploadFrm" method="post" action="<?php echo BBSE_COMMERCE_PLUGIN_WEB_URL;?>admin/proc/bbse-commerce-inven-dbinsert.exec.php" enctype="multipart/form-data">
			<input type="hidden" name="mType" id="mType" value="step01" />
			<div class="titleH5" style="margin:20px 0 10px 0; ">STEP 1. 제품 일괄 등록 정보 CSV 파일 업로드 </div>
		...
		<div id="csvUploadMsg" class="borderBox" style="background-color:#ffffff;line-height:30px;">
			제품 일괄 등록 파일(.csv)을 업로드 해 주세요.
		</div>
	</div>

	<div class="memberStep02" style="display:none;">
		<div class="clearfix" style="margin-top:30px;"></div>
		<div class="titleH5" style="margin:20px 0 10px 0;text-align:left;">STEP 2. 제품 일괄 데이터 저장 <span class="stepUseCheck" id="stepUseCheck-02" data-use="yes" data-target="memberStep02-contents" style="cursor:pointer;margin-left:64px;"><img src="<?php echo BBSE_COMMERCE_PLUGIN_WEB_URL;?>/images/switch_yes.png" align="absmiddle" /></span></div>

		<div class="memberStep02-contents">
			<form name="dbInsertFrm" id="dbInsertFrm" method="post" action="<?php echo BBSE_COMMERCE_PLUGIN_WEB_URL;?>admin/proc/bbse-commerce-inven-dbinsert.exec.php">
	...
	<div class="memberStep03" style="display:none;">
		...
		<div class="memberStep03-contents">
			...
			<form name="csvMakeFrm" id="csvMakeFrm" method="post" action="<?php echo BBSE_COMMERCE_PLUGIN_WEB_URL;?>admin/proc/bbse-commerce-inven-dbinsert.exec.php">
				<input type="hidden" name="mType" id="mType3" value="failCsvMake" />
		...
		jQuery("#dbUploadFrm").ajaxSubmit({
			...
			success: function(data) {
				...
				if(result['0']=='success'){
					...
					jQuery("#uploadResultArea").html("<strong>1. 파일 명 : </strong>"+result['1']+"<br /><strong>2. 제품 수 : </strong>"+str_comma(result['3'])+"개<br /><strong>3. 필드 수 : </strong>"+str_comma(result['4'])+ "개<br />");
					jQuery("#dbFile").val("");
	...
	function csvInsert() {
		...
		if(!jQuery("#cvsUploadFile").val()){
			alert("제품 일괄 등록 파일(csv)이 존재하지 않습니다.        ");
			return;
		}

		jQuery("#uploadResultMsg").show();

		if(confirm('해당 제품 일괄 등록 파일(csv)을 저장하시겠습니까?       ')){
			jQuery.ajax({
				type: 'post', 
				async: false, 
				url: '<?php echo BBSE_COMMERCE_PLUGIN_WEB_URL?>admin/proc/bbse-commerce-inven-dbinsert.exec.php', 
				...
				success: function(data){
					...
					if(result['0']=='success'){
						...
						msgStr +="<strong>2. 전체 제품 등록 수 : </strong>"+str_comma(result['2'])+" 건<br />";
						msgStr +="<strong>3. 저장 완료 제품 수 : </strong>"+str_comma(result['3'])+" 건<br />";
						
						if(result['4']>0) btnFailDown="<button type=\"button\" class=\"button-small red\" onclick=\"fail_data_download();\" style=\"height:25px;margin-left:50px;\">오류 데이터 CSV 다운로드</button>";
						msgStr +="<span class='emRed'><strong>5. 저장 실패 제품 수 : </strong>"+str_comma(result['4'])+" 건</span>"+btnFailDown+"<br />";
						
						if(result['4']>0){
							var failCount=result['5'].split("|");
							if(failCount['0']>0) msgStr +="&nbsp;&nbsp;<span class='emRed'>- 제품 고유 ID 값 없음 : "+str_comma(failCount['0'])+" 건</span> (회원 아이디 입력)<br />";
							if(failCount['1']>0) msgStr +="&nbsp;&nbsp;<span class='emRed'>- 해당창고, 해당제품 기등록 되어 있음 : "+str_comma(failCount['1'])+" 건</span> (중복 제거)<br />";
							if(failCount['2']>0) msgStr +="&nbsp;&nbsp;<span class='emRed'>- 제품코드 없음 : "+str_comma(failCount['2'])+" 건</span> (회원명 입력)<br />";
							if(failCount['3']>0) msgStr +="&nbsp;&nbsp;<span class='emRed'>- 현재 수량 없음 : "+str_comma(failCount['3'])+" 건</span> ("+pwTitle+" 입력)<br />";
							if(failCount['4']>0) msgStr +="&nbsp;&nbsp;<span class='emRed'>- 재고 알림수량 없음 : "+str_comma(failCount['4'])+" 건</span> ("+pwTitle+" 16자 이내로 입력)<br />";
							if(failCount['5']>0) msgStr +="&nbsp;&nbsp;<span class='emRed'>- 담당자 아이디없음 : "+str_comma(failCount['4'])+" 건</span> ("+pwTitle+" 16자 이내로 입력)<br />";
							if(failCount['6']>0) msgStr +="&nbsp;&nbsp;<span class='emRed'>- 창고 코드 없음 : "+str_comma(failCount['4'])+" 건</span> ("+pwTitle+" 16자 이내로 입력)<br />";

							jQuery("#cvsMakeOriginalFile").val(result['1']);
							jQuery("#failData").val(result['8']);
						}
